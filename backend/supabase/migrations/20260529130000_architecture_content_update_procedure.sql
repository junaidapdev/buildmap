-- Chunk 16 - Project Architecture Content Update Procedure
-- Date: 2026-05-29
-- Purpose: Persist an edited, per-section-regenerated, or per-decision-regenerated architecture. The
-- SPA sends the full new content_json; the save Edge Function renders content_markdown
-- deterministically and calls this procedure, which bumps the version and resets is_final so the
-- user must re-approve. Mirrors update_project_prd_content (Chunk 14).
--
...
begin
  ...
  -- An architecture must already exist; this procedure updates, it does not create.
  select id, version into v_existing_id, v_existing_version
  from public.project_documents
  where project_id = p_project_id and type = 'architecture';

  if v_existing_id is null then
    raise exception 'architecture does not exist for project %', p_project_id;
  end if;
...
grant execute on function public.update_project_architecture_content(uuid, jsonb, text) to authenticated;
