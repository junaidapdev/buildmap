...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const parsedRequest = SavePrdContentInputSchema.safeParse(body);
    ...
    if (projectError) {
      logger.error('prd_save_project_lookup_failed', {
        userId,
        projectId,
    ...
    const { data: updateMeta, error: rpcError } = await supabase.rpc('update_project_prd_content', {
    ...
    if (rpcError) {
      logger.error('prd_save_failed', { userId, projectId, code: rpcError.code });
      return fail(
        ERROR_CODES.INTERNAL,
        ERROR_MESSAGES.INTERNAL,
        HTTP_STATUS.INTERNAL_SERVER_ERROR,
    ...
    if (!parsedMeta.success) {
      logger.error('prd_save_meta_invalid', { userId, projectId });
      return fail(
        ERROR_CODES.INTERNAL,
        ERROR_MESSAGES.INTERNAL,
        HTTP_STATUS.INTERNAL_SERVER_ERROR,
    ...
    logger.info('prd_content_saved', { userId, projectId, version: parsedMeta.data.version });
    ...
    logger.error('prd_save_unhandled_error');
    return fail(
      ERROR_CODES.INTERNAL,
      ERROR_MESSAGES.INTERNAL,
      HTTP_STATUS.INTERNAL_SERVER_ERROR,
