...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const rateLimitCheck = await checkRateLimit(
      supabase,
      userId,
      GENERATION_FUNCTION_NAMES.GENERATE_PROJECT_BRIEF,
    );

    if (!rateLimitCheck.allowed) {
      logger.info('rate_limit_blocked', {
        userId,
        functionName: GENERATION_FUNCTION_NAMES.GENERATE_PROJECT_BRIEF,
    ...
    const { projectId: parsedProjectId, answers } = parsedRequest.data;
