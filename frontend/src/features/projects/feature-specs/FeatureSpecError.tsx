...
export function FeatureSpecError({ onRetry }: FeatureSpecErrorProps) {
  return (
    <section className="flex flex-col items-center py-16 text-center" role="alert">
      ...
      <h2 className="text-xl font-semibold">{FEATURE_SPEC_MESSAGES.ERROR_TITLE}</h2>
      <p className="mt-2 max-w-md text-muted-foreground">{FEATURE_SPEC_MESSAGES.ERROR_BODY}</p>
      <div className="mt-6">
        <Button onClick={onRetry}>{FEATURE_SPEC_MESSAGES.ERROR_RETRY}</Button>
      </div>
    </section>
  );
}
